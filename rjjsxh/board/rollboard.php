...
<section  id="noticeDetail">
    ...
    <div class=" mainsection">
        <div class="">
            <button class="btn btn-primary" id="changeboard" onclick="location.href = 'board.php'"> 点我返回普通留言</button>
            <div class="rollboard">
                <?php
                    //获取最大ID
                    ...
                    for($i = $maxID[0]; $i >=1; $i--){
                        ...
                        $top = rand(48, 100);     //距离顶部百分比
                        $speed = rand(5,20);    //速度
                        $color = rand(100000,999999);   //颜色
                        $font = rand(14,24);    //字体大小
                        if($text==""){
                            continue;
                        } else{
                            echo "<marquee direction='left' width='100%' onmouseover=this.stop() onmouseout=this.start() style='top:$top%;color:#$color;font-size:$font\px'  scrollamount=\"$speed\">$text[0]</marquee>";
                        }
                    }
                ?>
            </div>
